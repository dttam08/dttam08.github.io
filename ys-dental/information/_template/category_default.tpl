...
<title>{=$current_category_name=}｜町田市の歯医者｜ワイズデンタルオフィス</title>
<meta name="keywords" content="お知らせ,ブログ,休み,情報">
<meta name="description" content="町田市の歯医者「ワイズデンタルオフィス」の{=$current_category_name=}はこちらをご覧ください。当院のお休みやお口の健康に役立つ情報などを日々発信しています。お時間がありましたら、お気軽にチェックしていただけますと幸いです。">
...
<div id="wrapper">
   ...
   <header id="header">
    <div class="container">
      <h1 class="logo"><a href="https://www.ys-dental.com/"><img loading="lazy" src="../../images/logo.jpg" alt="町田市の歯医者「ワイズデンタルオフィス」、「{=$current_category_name=}」のページです。"></a></h1>
      <div class="h_right">
        <div class="h_info">
      ...
      <div class="section"> 
        ...
        <ONContributeSearch page="@$_GET['p']" limit="$limitNum" category="@$current_category_id">
          <div class="information_list">
            <ONContributeFetch>
              <div class="information_item"> <a href="../{=$url=}">
                <p class="information_itm_img">
                  <ONIf condition="$image1_Value"> <img src="{=$image1_Src=}" alt="{=$title=}" />
                    <ONElse>
                    <img src="../../images/under_img01.jpg" alt="{=$title=}" /> </ONIf>
                </p>
                <?php
                $dates = explode("/", $date);
              ?>
                <div class="information_itm_main">
                  <p class="information_itm_date">{=$dates[0].'.'=}{=$dates[1].'.'=}{=$dates[2]=}</p>
                  <p class="information_itm_ttl">{=mb_strimwidth($title, 0, 100, '…', 'UTF-8')=}</p>
                </div>
                </a> </div>
            </ONContributeFetch>
          </div>
