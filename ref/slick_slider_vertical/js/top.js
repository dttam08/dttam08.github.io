            ...
            $.ajax({
                ...
                'success': function(json) {
                    $.each(json.data, function(i, val) {
                        ...
                        var ttl = val.title;
                        if (vw > 641 && vw < 1366) {
                            ttl = ttl.length < 45 ? ttl : ttl.substr(0, 40) + '...';
                        }
                        ttl = ttl.length < 45 ? ttl : ttl.substr(0, 45) + '...';
                        var thumb = val.img02 ? $(val.img02).attr('src') : 'images/idx_b04_img.jpg';
                        var cate = val.txt02 ? '<p class="cate"><span>' + val.txt02 + '</span></p>' : '';
                        var item = $('<li class="idx_b04_item"><a  href="works/' + val.url + '"><p class="img"><img src="' + thumb + '" alt="' + ttl + '"></p>' + cate + '<p class="ttl">' + ttl + '</p></a></li>');
...
$(document).ready(function() {
    ...
    $('.open-popup').click(function() {
        var idpop = $(this).attr("data-pop");
        $("#"+idpop).addClass('open');
    });
    $(document).on('mousedown touchstart', function(e) {
        if ($(e.target).closest(".popup-inner, popup .popup-inner .close").length === 0) {
            if ($('.idx_popup').hasClass('open')) {
                $('.idx_popup').removeClass('open');
            }
        }
    });
    $('.idx_popup .popup-inner .close').click(function() {
        $('.idx_popup').removeClass('open');
    });
});